      ...
      <el-table :data="tableData">
        <el-table-column label="名称" min-width="220">
          <template #default="scope">
            <router-link
              :to="{ name: 'serviceAccount_detail', query: { serviceAccount: scope.row.name, namespace: scope.row.namespace } }"
            >
        ...
        <el-table-column fixed="right" label="操作" width="240">
          <template #default="scope">
            <el-button icon="view" type="primary" link size="small" @click="viewServiceAccount(scope.row)">查看</el-button>
...
import { getServiceAccountList, getServiceAccountRaw, deleteServiceAccount } from '@/api/kubernetes/serviceAccount'
...
export default {
  ...
  setup() {
    ...
    const getTableData = async() => {
      const table = await getServiceAccountList({ page: page.value, pageSize: pageSize.value, ...searchInfo })
      if (table.code === 0) {
    ...
    const viewServiceAccount = async(row) => {
      const result = await getServiceAccountRaw({ serviceAccount: row.name, namespace: row.namespace })
      if (result.code === 0) {
    ...
    const deleteFunc = async(row) => {
      ElMessageBox.confirm('此操作将永久删除该ServiceAccount, 是否继续?', '提示', {
        ...
        .then(async() => {
          const res = await deleteServiceAccount({ namespace: row.namespace, roleBinding: row.name })
          if (res.code === 0) {
            ElMessage({
              type: 'success',
              message: '删除成功!'
