...
func (jwtService *JwtService) SetRedisJWT(jwt string, userName string) (err error) {
	// 此处过期时间等于jwt过期时间
	timer := time.Duration(global.KOP_CONFIG.JWT.ExpiresTime) * time.Second
	err = global.KOP_REDIS.Set(context.Background(), userName, jwt, timer).Err()
	return err
}

func LoadAll() {
	var data []string
	err := global.KOP_DB.Model(&system.JwtBlacklist{}).Select("jwt").Find(&data).Error
	if err != nil {
		global.KOP_LOG.Error("加载数据库jwt黑名单失败!", zap.Error(err))
		return
	}
