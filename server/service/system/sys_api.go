...
func (apiService *ApiService) CreateApi(api system.SysApi) (err error) {
	if !errors.Is(global.KOP_DB.Where("path = ? AND method = ?", api.Path, api.Method).First(&system.SysApi{}).Error, gorm.ErrRecordNotFound) {
		return errors.New("存在相同api")
	}
	return global.KOP_DB.Create(&api).Error
}
...
func (apiService *ApiService) DeleteApi(api system.SysApi) (err error) {
	err = global.KOP_DB.Delete(&api).Error
...
func (apiService *ApiService) GetAPIInfoList(api system.SysApi, info request.PageInfo, order string, desc bool) (err error, list interface{}, total int64) {
	...
	db := global.KOP_DB.Model(&system.SysApi{})
...
func (apiService *ApiService) GetAllApis() (err error, apis []system.SysApi) {
	err = global.KOP_DB.Find(&apis).Error
	return
}
...
func (apiService *ApiService) UpdateApi(api system.SysApi) (err error) {
	...
	err = global.KOP_DB.Where("id = ?", api.ID).First(&oldA).Error
	...
	if err != nil {
		...
		if err != nil {
			...
			err = global.KOP_DB.Save(&api).Error
		}
	}
	return err
}
...
func (apiService *ApiService) DeleteApisByIds(ids request.IdsReq) (err error) {
	err = global.KOP_DB.Delete(&[]system.SysApi{}, "id in ?", ids.Ids).Error
	return err
}

func (apiService *ApiService) DeleteApiByIds(ids []string) (err error) {
	return global.KOP_DB.Delete(&system.SysApi{}, "id in ?", ids).Error
}
