...
func (n *NodeService) GetNodeDetail(name string) (*modelK8s.NodeDetail, error) {
	node, err := global.KOP_K8S_Client.CoreV1().Nodes().Get(context.TODO(), name, metaV1.GetOptions{})
	if err != nil {
		global.KOP_LOG.Error("获取节点信息失败", zap.Error(err))
		return nil, err
	}
	...
	if err != nil {
		global.KOP_LOG.Error("获取节点pod失败", zap.Error(err))
	}
...
func getNodePods(name string) (*coreV1.PodList, error) {
	...
	return global.KOP_K8S_Client.CoreV1().Pods(coreV1.NamespaceAll).List(context.TODO(), metaV1.ListOptions{
