...
func (i *DBApi) InitDB(c *gin.Context) {
	if global.KOP_DB != nil {
		global.KOP_LOG.Error("已存在数据库配置!")
		response.FailWithMessage("已存在数据库配置", c)
		return
	}
	var dbInfo request.InitDB
	if err := c.ShouldBindJSON(&dbInfo); err != nil {
		global.KOP_LOG.Error("参数校验不通过!", zap.Error(err))
		response.FailWithMessage("参数校验不通过", c)
		return
	}
	if err := initDBService.InitDB(dbInfo); err != nil {
		global.KOP_LOG.Error("自动创建数据库失败!", zap.Error(err))
...
func (i *DBApi) CheckDB(c *gin.Context) {
	if global.KOP_DB != nil {
		global.KOP_LOG.Info("数据库无需初始化")
		response.OkWithDetailed(gin.H{"needInit": false}, "数据库无需初始化", c)
		return
	} else {
		global.KOP_LOG.Info("前往初始化数据库")
