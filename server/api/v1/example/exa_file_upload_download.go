...
import (
	...
	exampleRes "github.com/pddzl/kubeops/server/model/example/response"
)
...
func (u *FileUploadAndDownloadApi) UploadFile(c *gin.Context) {
	...
	if err != nil {
		global.KOP_LOG.Error("接收文件失败!", zap.Error(err))
		response.FailWithMessage("接收文件失败", c)
		return
	}
	err, file = fileUploadAndDownloadService.UploadFile(header, noSave) // 文件上传后拿到文件路径
	if err != nil {
		global.KOP_LOG.Error("修改数据库链接失败!", zap.Error(err))
